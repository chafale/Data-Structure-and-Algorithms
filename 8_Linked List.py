...
class Solution:
    def splitListToParts(self, head: Optional[ListNode], k: int) -> List[Optional[ListNode]]:
        # finding the lenght of the linklist
        listLen = 0
        curr = head
        while curr:
            listLen += 1
        ...
        # find the base lenght - min element every partion will have
        # find how many node will have extra elements ==> len % k
        base_len, remainder = listLen // k, listLen % k
        ...
        for i in range(k):
            ...
            # add extra elements in the list
            for j in range(base_len - 1 + (1 if remainder else 0)):
                if not curr: break
                curr = curr.next

            # decrement no. of node having extra element
            remainder -= (1 if remainder else 0)

            # adjusting curr pointer
            if curr:
                tmp = curr.next
                curr.next = None
                curr = tmp
